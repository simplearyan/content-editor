        ...
        <div className="relative px-6 pt-12 pb-6">
          <div className="flex items-center gap-4 mb-8">
            <div className="relative shrink-0">
              ...
              <Avatar>
            {session?.user?.image && (
            <AvatarImage src={session?.user.image} alt={session?.user.name || "https://ferf1mheo22r9ira.public.blob.vercel-storage.com/avatar-01-n0x8HFv8EUetf9z6ht0wScJKoTHqf8.png"} />
            )}
            ...
            <div className="flex-1">
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{session?.user?.name || 'Admin'}</h2>
              <p className="text-zinc-600 dark:text-zinc-400">{session?.user?.email ? session?.user.email : session?.user?.githubUsername}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <button
              type="button"
              className="w-full flex items-center justify-between p-2 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 
               rounded-lg transition-colors duration-200"
            >
